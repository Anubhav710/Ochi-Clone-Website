...
import Montrial from "next/font/local";
...
const Navbar = () => {
  ...
  return (
    <motion.nav
      ...
      className="z-50 py-5 bg-white bg-opacity-10 backdrop-filter backdrop-blur-md   lg:px-14 px-5 w-full  fixed top-0 flex  items-center  justify-between "
    >
      {/* IMAGE CONTAINER  */}
      <div
        className={`${isActive ? "text-white" : "text-black"} lg:text-black`}
      >
        <svg
          width="72"
          height="30"
      ...
      <div className=" hidden lg:flex gap-5 ">
        {NavLinks.map((btn, indx) => (
          <NavButton
            className={`${NM.className} ${indx === 4 && "ml-80"} `}
            key={indx}
          >
            {btn.name}
          </NavButton>
